...
Kısa Kod Sayfaları, MHM Rentiva'nın tüm frontend (ön yüz) fonksiyonlarını WordPress ile entegre eden mimarinin merkezidir. Sistemimizde aktif olarak görev yapan **28 adet kısa kod (shortcode)** bulunmaktadır. Bu araçları ister klasik (metin), ister sayfa oluşturucu entegrasyonlarıyla sitenizin her yerine konumlandırabilirsiniz.

---

## 🏗️ Gutenberg Blokları ve Elementor Desteği

MHM Rentiva, modern WordPress mimarisini benimsemektedir. Bu nedenle burada yer alan **28 listelenmiş kısa kod**, arka planda standart bir "Render Çıktısı" oluşturur.
...
## 📋 Aktif 28 Kısa Kod ve Kullanımları
...
**3. Araç Detayları:** Tekil bir aracın tüm özelliklerini listeler.
...
**6. Araçlar Grid (Izgara Görünümü):**
...
**7. Birleşik Arama (Unified Search):** Karma kiralama formu.
...
**8. Arama Sonuçları Görünümü:** Aramadan dönen araç grid formatı.
...
**9. Araç Karşılaştırma:** Özelliklerin kıyaslanması.
...
**10. Müşteri Ana Paneli:** Müşteri özet ekranı.
...
**13. Ödeme Geçmişi:** İşlem makbuzu/Transaction tutanakları.
...
**14. Komisyon Çözümleyici:** Affiliate / Satıcılara özel sistem.
...
**15. Transfer Arama Formu:** Yolcu alma / bırakma seçimi.
...
**16. Transfer Sonuçları:** Transfer tarifeleri listesi.
...
**17. Popüler Rotalar (v4.34.0+):** VIP transfer rotalarının ana sayfa vitrini; tıklanabilir kartlar transfer aramaya deep-link yapar.
*   **Kısa Kod:** `[rentiva_popular_routes]`
*   **Temel Parametreler:** `limit`, `columns`, `order`, `featured_only`, `filter_origin_city`
*   **Örnek Kullanım:** `[rentiva_popular_routes columns="3" limit="6" order="featured"]`
*   **Referans:** [Popüler rotalar](./popular-routes)
...
**25. Bayi Profil Sayfası (v4.37.0+):**
*   **Kısa Kod:** `[rentiva_vendor_profile]`
*   **Temel Parametreler:** `slug`, `show_badge`, `show_about`, `show_vehicles`, `show_reviews`, `show_location`, `max_vehicles`, `max_reviews`
*   **Örnek Kullanım:** `[rentiva_vendor_profile slug="akif-otomotiv"]`
*   **Referans:** [Bayi profil sayfası](./vendor-profile)

**26. Bayi Dizini (v4.38.0+):**
*   **Kısa Kod:** `[rentiva_vendor_directory]`
*   **Temel Parametreler:** `per_page`, `default_sort`, `show_filter_bar`, `show_breadcrumb`, `show_pagination`, `empty_message`
*   **Örnek Kullanım:** `[rentiva_vendor_directory per_page="12" default_sort="rating"]`
*   **Referans:** [Bayi dizini](./vendor-directory)

**27. Bayi Rezervasyon Gelen Kutusu (panel-yalnız):**
*   **Kısa Kod:** `[rentiva_vendor_bookings]`
*   **Temel Parametreler:** `limit` (sayfa boyutu, varsayılan 10)
*   **Örnek Kullanım:** `[rentiva_vendor_bookings limit="10"]`
*   **Referans:** [Bayi rezervasyon listesi](./vendor-bookings)

**28. Bayi Defteri (panel-yalnız, finansal işlemler):**
*   **Kısa Kod:** `[rentiva_vendor_ledger]`
*   **Temel Parametreler:** Yok (yalnızca URL filtreleri — `paged`, tür, tarih aralığı)
*   **Örnek Kullanım:** `[rentiva_vendor_ledger]`
*   **Referans:** [Bayi defteri](./vendor-ledger)
...
### Bölüm Özeti
- **28 Kısa Kod:** Projenizin rezervasyondan b2b'ye her noktasını içerir.
