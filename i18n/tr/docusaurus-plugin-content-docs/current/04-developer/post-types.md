...
- **İlişki:** Her payout kaydı, veritabanındaki `mhmrentiva_ledger` satırlarıyla Transaction ID üzerinden eşleşir.
...
- **Thread Yönetimi:** `_mhmrentiva_thread_id` meta anahtarı üzerinden mesajları gruplandırır.
...
- **`mhmrentiva_vehicle`:** Kiralik araç portfoyu.
- **`mhmrentiva_booking`:** Rezervasyon kayıtlari ve takvimi.
- **`mhmrentiva_addon`:** Araç ek hizmetleri (cocuk koltugu, GPS, sigorta vb.).

---

## 5. Vendor Marketplace Kayıt Turleri (Pro)

### `mhmrentiva_vendor_app` (Vendor Başvurulari)
Vendor marketplace kapsaminda satici başvurularini yonetir.
- **CPT Slug:** `mhmrentiva_vendor_app` (14 karakter — WordPress 20 karakter limitine uygun). **NOT:** `mhmrentiva_vendor_application` DEGiL.
...
- Finansal kayıtlar (`mhmrentiva_payout`) asla direkt veritabanından silinmez; statu degisikligi ile arsivlenir.
- Loglar (`mhmrentiva_app_log`) performans için duzenli olarak temizlenir.
- Vendor başvurulari `mhmrentiva_vendor_app` CPT'sinde saklanir (14 karakter, WP 20-char limiti).

## Değişiklik Günlüğü
| Tarih | Sürüm | Not |
|---|---|---|
| 27.03.2026 | 4.23.0 | `mhmrentiva_addon` ve `mhmrentiva_vendor_app` CPT'leri, vendor marketplace detaylari eklendi. |
| 19.03.2026 | 4.21.2 | mhmrentiva_payout, mhmrentiva_message ve LogRetention detaylari eklendi. |
