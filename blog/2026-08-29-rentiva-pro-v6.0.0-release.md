...
**One filter is gone rather than renamed.** `mhm_rentiva_vendor_apply_endpoint_slug` no longer exists. The vendor-application endpoint's address is now fixed to your site's language and treated as an identifier rather than a label, so there is nothing left to filter.
